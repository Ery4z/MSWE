...
class _BaseWorldFolder(object):
    ...
    def __init__(self, world_folder):
        ...
        self.set_entitiesfiles(self.get_filenames(subfolder="entities"))
    ...
    def get_filenames(self, subfolder="region"):
        ...
        return list(
            glob.glob(
                os.path.join(
                    self.worldfolder, subfolder, "r.*.*." + self.extension
                )
            )
        )

    def set_entitiesfiles(self, filenames):
        for filename in filenames:
            # Assume that filenames have the name r.<x-digit>.<z-digit>.<extension>
            m = re.match(
                r"r.(\-?\d+).(\-?\d+)." + self.extension,
            ...
            if m:
                ...
                pass
            self.entitiesfiles[(x, z)] = filename
    ...
    def get_entities(self, x, z):
        """Get a region using x,z coordinates of a region. Cache results."""
        if (x, z) not in self.entities:
            if (x, z) in self.entitiesfiles:
                self.entities[(x, z)] = region.RegionFile(
                    self.entitiesfiles[(x, z)]
                )
            else:
                # Return an empty RegionFile object
                # TODO: this does not yet allow for saving of the region file
                ...
                # and add the fie to self.regionfiles
                self.entities[(x, z)] = region.RegionFile()
            self.entities[(x, z)].loc = Location(x=x, z=z)
        return self.entities[(x, z)]
        ...
        for x, z in self.regionfiles.keys():
            ...
            if (x, z) in self.regions:
                ...
                regionfile = region.RegionFile(
                    self.regionfiles[(x, z)], chunkclass=self.chunkclass
                )
    ...
    def get_nbt(self, x, z):
        ...
        if (rx, rz) not in self.regions and (rx, rz) not in self.regionfiles:
            raise InconceivedChunk(
                "Chunk %s,%s is not present in world" % (x, z)
            )
...
class BoundingBox(object):
    ...
    def __init__(
        self, minx=None, maxx=None, miny=None, maxy=None, minz=None, maxz=None
    ):
