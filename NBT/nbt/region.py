...
class RegionFile(object):
    ...
    def __init__(self, filename=None, fileobj=None, chunkclass=None):
        ...
        self.file = None
        self.filename = None
        self.region_coord = (filename.split(".")[-3], filename.split(".")[-2])
        ...
        if filename:
            ...
            raise ValueError(
                "RegionFile(): Need to specify either a filename or a file object"
            )
    ...
    def _parse_header(self):
        ...
        if self.size == 0:
            ...
            raise NoRegionHeader(
                "The region file is %d bytes, too small in size to have a header."
                % self.size
            )
    ...
    def _parse_chunk_headers(self):
        for x in range(32):
            for z in range(32):
                ...
                try:
                    self.file.seek(
                        m.blockstart * SECTOR_LENGTH
                    )  # offset comes in sectors of 4096 bytes
                    length = unpack(">I", self.file.read(4))
                    m.length = length[
                        0
                    ]  # unpack always returns a tuple, even unpacking one element
    ...
    def get_blockdata(self, x, z):
        ...
        if m.status == STATUS_CHUNK_NOT_CREATED:
            ...
            raise RegionHeaderError(
                "Chunk %d,%d is partially/completely outside the file" % (x, z)
            )
        ...
        try:
            ...
            if m.compression == COMPRESSION_GZIP:
                ...
                raise ChunkDataError(
                    "Unknown chunk compression/format (%s)" % m.compression
                )
        ...
        if err:
            ...
            if m.status == STATUS_CHUNK_MISMATCHED_LENGTHS:
                raise ChunkHeaderError(
                    "The length in region header and the length in the header of chunk %d,%d are incompatible"
                    % (x, z)
                )
            elif m.status == STATUS_CHUNK_OVERLAPPING:
                raise ChunkHeaderError(
                    "Chunk %d,%d is overlapping with another chunk" % (x, z)
                )
            else:
    ...
    def write_blockdata(self, x, z, data, compression=COMPRESSION_ZLIB):
        ...
        sector = self._find_free_location(
            free_sectors, nsectors, preferred=current.blockstart
        )
        ...
        for s in range(
            current.blockstart,
            min(current.blockstart + current.blocklength, len(free_sectors)),
        ):
            if free_sectors[s]:
                # zero sector s
    ...
    def denormalize_entities_coord(self, x, z, nbt_file):
