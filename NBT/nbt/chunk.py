...
class McRegionChunk(Chunk):
    def __init__(self, nbt):
        ...
        self.blocks = BlockArray(
            self.chunk_data["Blocks"].value, self.chunk_data["Data"].value
        )
...
class AnvilSection(object):
    def __init__(self, nbt, version):
        ...
        if version == 0 or version == 1343:  # 1343 = MC 1.12.2
            ...
            version >= 2566 and version <= 2730
        ):  # MC 1.16.0 to MC 1.17.2 (latest tested version)
    ...
    def _init_index_unpadded(self, nbt):
        ...
        for _ in range(0, 4096):
            ...
            if num_bits <= bits_left:
                ...
                next_long = (
                    next_long & (pow(2, remaining_bits) - 1)
                ) << bits_left
    ...
    def _init_index_padded(self, nbt):
        ...
        if last_state_elements == 0:
            last_state_elements = indexes_per_element
    ...
    def __init__(self, nbt):
        ...
        try:
            ...
            if version != 1343 and not (version >= 1631 or version <= 2730):
                raise NotImplementedError(
                    "DataVersion %d not implemented" % (version,)
                )
        except KeyError:
            version = 0
        ...
        if "Sections" in self.chunk_data:
            for s in self.chunk_data["Sections"]:
                if (
                    "BlockStates" in s.keys()
                ):  # sections may only contain lighting information
...
class BlockArray(object):
    ...
    def __init__(self, blocksBytes=None, dataBytes=None):
        ...
        if isinstance(blocksBytes, (bytearray, array.array)):
            ...
            self.blocksList = [
                0
            ] * 32768  # Create an empty block list (32768 entries of zero (air))
        ...
        if isinstance(dataBytes, (bytearray, array.array)):
            ...
            self.dataList = [
                0
            ] * 16384  # Create an empty data list (32768 4-bit entries of zero make 16384 byte entries)
    ...
    def generate_heightmap(self, buffer=False, as_array=False):
        ...
        if buffer:
            return BytesIO(
                pack(">i", 256) + self.generate_heightmap()
            )  # Length + Heightmap, ready for insertion into Chunk NBT
        else:
            bytes = []
            for z in range(16):
                for x in range(16):
    ...
    def set_block(self, x, y, z, id, data=0):
        ...
        if offset % 2 == 1:
            ...
            self.dataList[index] = (b & 240) + (
                data & 15
            )  # modify lower bits, leaving higher bits in place
        else:
            # offset is even
            index = offset // 2
            b = self.dataList[index]
            self.dataList[index] = (b & 15) + (
                data << 4 & 240
            )  # modify ligher bits, leaving lower bits in place
    ...
    def get_block(self, x, y, z, coord=False):
        ...
        offset = (
            y + z * 128 + x * 128 * 16
            if (coord == False)
            else coord[1] + coord[2] * 128 + coord[0] * 128 * 16
        )
